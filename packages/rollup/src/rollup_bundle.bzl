...
def _rollup_bundle(ctx):
    ...
    ctx.actions.run(
        ...
        execution_requirements = {
            # Never schedule this action remotely because it's not working on RBE.
            # See email thread with RBE team
            # Error looks like:
            # (20:53:40) ERROR: /workdir/packages/rollup/test/plugins/BUILD.bazel:4:1: Couldn't build file packages/rollup/test/plugins/bundle/input.js: Bundling JavaScript packages/rollup/test/plugins/bundle [rollup] failed (Exit 34). Note: Remote connection/protocol failed with: execution failed java.io.FileNotFoundException: /var/lib/buildkite-agent/.cache/bazel/_bazel_buildkite-agent/ec321eb2cc2d0f8f91b676b6d4c66c29/execroot/build_bazel_rules_nodejs/bazel-out/k8-fastbuild/bin/packages/rollup/test/plugins/bundle/input.js.tmp (No such file or directory)
            #	...	at com.google.devtools.build.lib.remote.AbstractRemoteActionCache.moveOutputsToFinalLocation(AbstractRemoteActionCache.java:333)
            "local": "1",
        },
    )
