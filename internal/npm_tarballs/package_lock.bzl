...
def _fetch_tarball(repository_ctx, bzl_out, packages):
    ...
    for (name, dep) in packages["dependencies"].items():
        ...
        bzl_out.extend([
            " " * 4 + "npm_fetch_tarball(",
            _attr("name", _repo_name(name, dep["version"])),
            _attr("filename", _escape(name) + "-" + dep["version"] + ".tgz"),
            _attr("integrity", dep["integrity"]),
            _attr("package_name", name),
            _attr("url", dep["resolved"]),
            " " * 4 + ")",
        ])
...
def _translate_package_lock(repository_ctx):
    ...
    bzl_content = [
        "\"Generated by package_lock.bzl from %s\"" % str(repository_ctx.attr.package_lock),
        "",
        """load("@build_bazel_rules_nodejs//internal/npm_tarballs:package_lock.bzl", "npm_fetch_tarball")""",
        "",
    ...
    # recurse one level deeper (maybe we need to do more?)
    version_lookup = {}
...
def _npm_fetch_tarball(repository_ctx):
    ...
    build_content = [
        ...
        "    src = \"%s\"," % repository_ctx.attr.filename,
        "    package_name = \"%s\"," % repository_ctx.attr.package_name,
        "    deps = [],",
        ")",
    ]
    repository_ctx.file("BUILD.bazel", "\n".join(build_content))

npm_fetch_tarball = repository_rule(
    doc = """Describe how to fetch a single tarball from npm""",
    implementation = _npm_fetch_tarball,
    attrs = {
        "url": attr.string(),
